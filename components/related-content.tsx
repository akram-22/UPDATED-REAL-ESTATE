...
function SectionShell({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "24px", border: "1px solid #e5e7eb" }}>
      <h2 style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "16px", fontWeight: "700", color: "#0a1628", marginBottom: "16px", letterSpacing: "1px" }}>{title}</h2>
      {children}
    </div>
  )
}
  ...
  return (
    <div onClick={() => router.push(`/properties/${property.id}`)}
      ...
      <div style={{ padding: "10px 12px" }}>
        <div style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "13px", fontWeight: 700, color: "#0a1628", marginBottom: "2px", lineHeight: "1.3" }}>{property.title}</div>
        <div style={{ fontSize: "11px", color: "#9ca3af", marginBottom: "4px" }}>📍 {property.location}</div>
        <div style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "13px", fontWeight: 700, color: "var(--gold)" }}>{property.price}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div onClick={() => router.push(`/blog/${article.slug}`)}
      ...
      <div style={{ padding: "12px" }}>
        <div style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "13px", fontWeight: 700, color: "#0a1628", lineHeight: "1.4" }}>{article.title}</div>
      </div>
    </div>
  )
}
...
export function RelatedArticles({ article }: { article: Article }) {
  ...
  return (
    <div style={{ marginTop: "16px" }}>
      <h2 style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "18px", fontWeight: 700, color: "#0a1628", marginBottom: "16px" }}>Voir aussi</h2>
