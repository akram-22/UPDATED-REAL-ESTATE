    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-navy/95 backdrop-blur-md shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-5 md:px-8 h-16 flex items-center justify-between">
        <Link href="/" className="font-serif text-xl font-bold text-white tracking-tight">
          {footer.brandName}
          <span className="block text-[10px] tracking-widest uppercase font-medium mt-[-2px]" style={{ color: "var(--gold)" }}>{footer.brandTagline}</span>
        </Link>

        <nav className="hidden md:flex items-center gap-6">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-white/70 hover:text-white text-sm font-medium transition-colors duration-150">{l.label}</a>
          ))}
      ...
      {mobileOpen && (
        <div className="md:hidden bg-navy border-t border-white/10 px-5 py-4 flex flex-col gap-3">
          {links.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className="text-white/80 hover:text-white text-sm font-medium py-1">{l.label}</a>
          ))}
          <a href={WA} target="_blank" rel="noopener noreferrer" className="whatsapp-btn text-sm justify-center py-2.5 mt-1">
            <WhatsAppIcon />WhatsApp
          </a>
        </div>
      )}
