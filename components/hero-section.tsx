    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-navy">
      <div className="absolute inset-0">
        {hero.backgroundImage && (
          <img src={hero.backgroundImage} alt="Vue panoramique d'Oran" className="w-full h-full object-cover opacity-30" />
        )}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-5 md:px-8 text-center pt-24 pb-16">
        <div className="inline-flex items-center gap-2 bg-white/10 border border-gold/40 backdrop-blur-sm rounded-full px-4 py-1.5 mb-8">
          ...
          <span className="text-gold text-xs font-medium tracking-widest uppercase">{hero.badge}</span>
        </div>
        ...
        <h1 className="font-serif text-4xl md:text-6xl lg:text-7xl text-white font-bold leading-tight mb-6 text-balance">
          {hero.headline.includes(",") ? (
            <>{hero.headline.split(",")[0]},<br /><span className="text-gold">{hero.headline.split(",").slice(1).join(",")}</span></>
          ) : hero.headline}
        </h1>

        <p className="text-white/70 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed mb-10 text-pretty">{hero.subheadline}</p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a href={WA} target="_blank" rel="noopener noreferrer" className="whatsapp-btn text-base px-8 py-4 w-full sm:w-auto justify-center">
            <WaIcon />{hero.ctaPrimaryText}
          </a>
          <a href="#properties" className="btn-outline-gold text-base px-8 py-4 w-full sm:w-auto justify-center border-white/40 text-white hover:bg-white/10 hover:border-white" style={{ color: "white", borderColor: "rgba(255,255,255,0.4)" }}>
