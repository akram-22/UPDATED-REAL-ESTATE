...
import { RelatedArticles } from "@/components/related-content"
...
export function BlogArticleClient() {
  ...
  if (!article) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", color: "#6b7280" }}>Article introuvable</p></div>
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f9fafb" }}>
      <header style={{ backgroundColor: "#0a1628", borderBottom: "1px solid rgba(201,168,76,0.2)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "0 20px", height: "60px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <button onClick={() => router.push("/blog")} style={{ display: "flex", alignItems: "center", gap: "8px", color: "rgba(255,255,255,0.7)", background: "none", border: "none", cursor: "pointer", fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "13px" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "24px 20px 80px" }}>
        <div style={{ marginBottom: "24px" }}>
          <Breadcrumb items={[{ name: "Blog", path: "/blog" }, { name: article.title, path: `/blog/${article.slug}` }]} />
        </div>
        {article.cover_image && (
        ...
        <div style={{ color: "var(--gold)", fontSize: "11px", letterSpacing: "3px", fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", marginBottom: "12px" }}>
        ...
        <h1 style={{ fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", fontSize: "clamp(28px,4vw,42px)", fontWeight: "800", color: "#0a1628", lineHeight: "1.2", letterSpacing: "-0.03em", marginBottom: "20px" }}>{article.title}</h1>
        ...
        {article.tags && article.tags.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "32px", paddingTop: "24px", borderTop: "1px solid #e5e7eb" }}>
            {article.tags.map((t) => (
              <span key={t} style={{ fontSize: "12px", fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", padding: "5px 12px", borderRadius: "100px", border: "1px solid #e5e7eb", color: "#6b7280" }}>#{t}</span>
            ))}
          </div>
        )}
        ...
        <div style={{ marginTop: "48px", padding: "24px", backgroundColor: "#0a1628", borderRadius: "12px", borderTop: "3px solid var(--gold)", textAlign: "center" }}>
          <p style={{ color: "rgba(255,255,255,0.7)", fontFamily: "Plus Jakarta Sans, Inter, system-ui, sans-serif", marginBottom: "16px" }}>Vous avez un projet immobilier à Oran ?</p>
