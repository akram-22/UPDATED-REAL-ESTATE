    ...
    <AdminShell>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm mt-0.5">
          {leads.length} contact(s) reçu(s) — <span className="text-red-600 font-medium">{unread} non lu(s)</span>
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {leads.map((lead) => (
            <div key={lead.id} className={`bg-card border rounded-xl p-5 flex flex-col sm:flex-row gap-4 transition-all ${lead.read ? "border-border" : "border-gold/40 bg-gold/5"}`}>
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {lead.details && <p className="text-sm text-foreground bg-secondary rounded-lg px-3 py-2 text-pretty">{lead.details}</p>}
              ...
              <div className="flex sm:flex-col gap-2 sm:items-end justify-end">
                <a href={`https://wa.me/${lead.phone.replace(/\D/g, "")}?text=Bonjour%20${encodeURIComponent(lead.name)}%2C%20je%20suis%20Chabane%20de%20Chabano%20Properties.`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-green-500 text-white hover:bg-green-600 transition-colors font-medium whitespace-nowrap">
