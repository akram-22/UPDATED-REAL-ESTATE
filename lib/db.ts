...
import type { Property, Lead, SiteContent } from "./types"
...
export async function createProperty(data: Omit<Property, "id" | "created_at" | "updated_at">): Promise<Property | null> {
  const { data: result, error } = await supabase
    .from("properties").insert([{ ...data, updated_at: new Date().toISOString() }]).select().single()
  if (error) { console.error(error); return null }
  return result
}
...
export async function updateProperty(id: string, data: Partial<Property>): Promise<Property | null> {
  const { data: result, error } = await supabase
    .from("properties").update({ ...data, updated_at: new Date().toISOString() })
...
export async function createLead(data: Omit<Lead, "id" | "created_at" | "read">): Promise<Lead | null> {
  const { data: result, error } = await supabase
    .from("leads").insert([{ ...data, read: false }]).select().single()
  if (error) { console.error(error); return null }
  return result
}
...
export async function getSiteContent(): Promise<SiteContent> {
  const { data, error } = await supabase
    .from("site_content").select("content").eq("id", 1).single()
